...
void makeROC (TString text_file, TString histName, TString label);  
...
void makeNormalised (TString text_file, TString histName, TString normName, int rebin) {
  ...
  for (auto& ln: lines) {
    ...
    irow++;
    TH1D *hi = (TH1D*)(file->Get(histName));
    if (irow == 1)       sigInt = hi->Integral();
    hi->Scale(sigInt/hi->Integral());
    hi->Rebin(rebin);
    ...
    if (irow == 1) {
      hi->SetLineWidth(3);
      hi->Draw("HIST");
      hi->GetYaxis()->SetRangeUser(0.0,10.0);
    }
...
void makeROC (TString text_file, TString histName, TString label) {
  ...
  for (size_t ib = 1; ib <= nBins; ++ib) {
    int irow = 0;
    float S = hvec[0]->Integral(ib, nBins);
    float B = 0.0;
    float allBkgInt = 0.0;
    for (auto& h: hvec){
      irow++;
  ...
  gStyle->SetOptStat(0);
  TCanvas *cst = new TCanvas("cst","morm hists",1600,1200);
  cst->SetGridx();
  cst->SetGridy();
  cst->Divide(2,2);
  cst->cd(1);
